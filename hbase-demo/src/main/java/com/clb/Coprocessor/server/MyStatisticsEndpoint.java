...
public class MyStatisticsEndpoint extends MyStatisticsInterface.myStatisticsService implements Coprocessor, CoprocessorService {
    ...
    public void getStatisticsResult(RpcController controller, MyStatisticsInterface.getStatisticsRequest request, RpcCallback<MyStatisticsInterface.getStatisticsResponse> done) {
        ...
        if (STATISTICS_COUNT.equals(type)) {
            ...
            try {
                ...
                LOG.error("error happend when count in "+ this.envi.getRegionInfo().getRegionNameAsString()+ " error is " + ioe);
            } finally {
                if (scanner != null) {
                    try {
                        scanner.close();
        ...
        // 当进行指定列值统计的时候
            ...
            if (!StringUtils.isBlank(famillyName)
                ...
                try {
                    ...
                    LOG.error("error happend when count in "+ this.envi.getRegionInfo().getRegionNameAsString()+ " error is " + e);
                } finally {
                    if (scanner != null) {
                        try {
                            scanner.close();
